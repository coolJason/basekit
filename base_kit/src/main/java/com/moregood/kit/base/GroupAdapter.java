...
public abstract class GroupAdapter<C extends RecyclerView.ViewHolder, S extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final int TYPE_HEADER = 1;
    private static final int TYPE_CONTENT = 0;
    //存放班级header布局所在的position
    protected List<Integer> mHeaderIndex = new ArrayList<>();
    ...
    public int getHeaderPosition(int position) {
        if (position >= 0 && position < mHeaderIndex.size()) {
            return mHeaderIndex.get(position);
        }
        return -1;
    }
