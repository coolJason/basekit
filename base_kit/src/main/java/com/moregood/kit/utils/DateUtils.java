...
public final class DateUtils {
    ...
    public static final String yyyyMMddHHmmss_3 = "yyyy年M月d日 HH:mm";
    ...
    private static int toInt(final String str, final int defaultValue) {
        ...
        try {
            ...
            Logger.d(TAG, e, "toInt");
        }
        return defaultValue;
    }

    public static String getYearMonthDay() {
        return String.format("%d-%02d-%02d", DateUtils.getYear(), DateUtils.getMonth(), DateUtils.getDay());
    }
     ...
     * @author wxy
     */
    public static String formatBeforeTime(Date ctime, String format) {
        String r = "";
...
//            r = df.format(new Date(ctimelong));
        }
        return r;
    }

    /**
     * 显示时间，时间在今天就返回时间hh:mm; 否则返回yyyy-MM-dd  使用场景：一般在明细列表中使用
     * @param ctime
     * @return
     */
    public static String formatDetailTime(Date ctime){
        String r = "";
        if(ctime==null)return r;

        Date currentTime = new Date();
        String currentDate = DateUtils.formatDate(currentTime, DateUtils.yyyyMMdd);

        String result = DateUtils.formatDate(ctime, DateUtils.yyyyMMdd);

        if (TextUtils.equals(result,currentDate)){
            return DateUtils.formatDate(ctime, DateUtils.HHmm);
        }
        return result;
    }
