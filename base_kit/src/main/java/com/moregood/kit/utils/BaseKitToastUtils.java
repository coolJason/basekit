 ...
 */
public class BaseKitToastUtils {
    ...
    public static void showCenterToast(String title, String message, int type, int duration) {
        ...
        showToast(BaseApplication.getInstance(), title, message, type, duration, Gravity.CENTER);
    }
    ...
    public static void showBottomToast(String title, String message, int type, int duration) {
        ...
        showToast(BaseApplication.getInstance(), title, message, type, duration, Gravity.BOTTOM);
    }
    ...
    public static void showToast(Context context, String title, String message, int type, int duration, int gravity) {
        ...
        //保证在主线程中展示toast
        BaseKitThreadManager.getInstance().runOnMainThread(() -> {
            if(toast != null) {
                toast.cancel();
            }
    ...
    private static Toast getToast(Context context, String title, String message, int type, int duration, int gravity) {
        ...
        View toastView = LayoutInflater.from(context).inflate(R.layout.base_toast_layout, null);
        ...
        if(type == SUCCESS) {
            ivToast.setImageResource(R.drawable.ic_success_base);
        }else if(type == FAIL) {
            ivToast.setImageResource(R.drawable.ic_warn_base);
        }else {
            ivToast.setVisibility(View.GONE);
        }
        int yOffset = 0;
        if(gravity == Gravity.BOTTOM || gravity == Gravity.TOP) {
            yOffset = (int) CommonUtils.dip2px(context, 50);
        }
